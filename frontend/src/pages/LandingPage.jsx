...
const LandingPage = () => {
  ...
  // ✅ Listen for login/logout changes (cross-tab)
  useEffect(() => {
    ...
    <div className="relative min-h-screen w-full overflow-x-hidden bg-[#0e0e10] text-white font-rajdhani">
        ...
        >
          ...
          <div className="flex flex-col justify-center text-center md:text-left space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 pt-4">
              ...
              >
                🚀 {user ? "Go to Playground" : "Try It Now – Free"}
              </button>

              {/* Show Demo/SignUp only when not logged in */}
              {!user && (
                <button
      ...
      <section id="how-it-works" className="py-16 bg-[#0e0e10] text-center relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-14 max-w-7xl mx-auto px-6">
          {[
            { img: "/assets/select.png", title: "Select", desc: "Choose from multiple AI models for comparison." },
            { img: "/assets/prompt.png", title: "Prompt", desc: "Enter your query once to test across models." },
            { img: "/assets/compare.png", title: "Compare", desc: "Analyze and choose the best model output." },
          ].map((step, i) => (
      ...
      {/* ✅ FEATURES */}
      <GlowingSeparator className="my-16" style={{ maxWidth: "1440px", margin: "0 auto" }} />
      ...
      <section className="py-16 bg-[#0e0e10] text-center relative z-10">
        <h2 className="text-3xl md:text-4xl font-spacegrotesk font-bold mb-4">
          {user ? "Manage Your Plan" : "Start Free – Upgrade Anytime"}
        </h2>
        <p className="text-blue-200 mb-8">
          {user
            ? `Welcome back, ${user.username || "User"}! Manage or upgrade your subscription below.`
        ...
        <div className="max-w-md mx-auto bg-[#1a1a1d]/80 p-6 rounded-xl border border-white/10 hover:border-[#2bd4c5]/40 shadow-lg transition-all">
          <h3 className="text-xl font-bold mb-2">{user ? "Your Current Plan" : "Free Trial"}</h3>
          <p className="text-blue-200 mb-4">
            {user
              ? "You are currently on a free plan. Upgrade to access premium features!"
